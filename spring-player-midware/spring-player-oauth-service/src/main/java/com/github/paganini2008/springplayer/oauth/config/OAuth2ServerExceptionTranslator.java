...
/**
 * 
 * OAuth2ServerExceptionTranslator
 *
 * @author Fred Feng
 * @version 1.0.0
 */
@Slf4j
public class OAuth2ServerExceptionTranslator implements WebResponseExceptionTranslator<OAuth2Exception> {
	...
	private ResponseEntity<OAuth2Exception> handleException(Exception e, ErrorCode errorCode, int statusCode) {
		...
		ResponseEntity<OAuth2Exception> response = new ResponseEntity<OAuth2Exception>(new OAuth2ServerException(errorCode), headers,
				httpStatus);
		return response;
	}
