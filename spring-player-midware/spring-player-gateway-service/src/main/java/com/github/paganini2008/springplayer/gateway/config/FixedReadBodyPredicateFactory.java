...
/**
 * 修复ReadBodyPredicateFactory关于POST请求无请求体的接口不响应的问题<br/>
 * <br/>
 * 
 * Predicate that reads the body and applies a user provided predicate to run on
 * the body. The body is cached in memory so that possible subsequent calls to
 * the predicate do not need to deserialize again.
 * 
 * @author Fred Feng
 * @version 1.0.0
 */
@Component
public class FixedReadBodyPredicateFactory extends AbstractRoutePredicateFactory<FixedReadBodyPredicateFactory.Config> {

	private static final Logger log = LoggerFactory.getLogger(FixedReadBodyPredicateFactory.class);
	...
	public FixedReadBodyPredicateFactory() {
		super(Config.class);
		this.messageReaders = HandlerStrategies.withDefaults().messageReaders();
	}

	public FixedReadBodyPredicateFactory(List<HttpMessageReader<?>> messageReaders) {
		super(Config.class);
