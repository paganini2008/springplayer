...
public class JobManager {
	...
	@Value("${spring.player.quartz.groupName:default-job-group-0}")
	private String defaultJobGroupName;

	@Value("${spring.player.quartz.triggerGroupName:default-trigger-group-0}")
	...
	public void addJob(String name, Class<?> jobClass, String description, long delayInMs, long intervalInMs, int repeatCount,
		...
		log.info("Add job '{}' successfully.", name);
	}

	public void addDelayedJob(String name, Class<?> jobClass, String description, long delayInMs, Map<String, Object> kwargs)
			throws Exception {
		JobDetail jobDetail = JobBuilder.newJob(DispatcherJobBean.class).withIdentity(name, defaultJobGroupName)
				.withDescription(description).build();
		JobDataMap dataMap = jobDetail.getJobDataMap();
